...
library(tidyverse)
setwd("C:/Users/Cam McMullen/Documents/Carleton_classes/Carleton_4th_year/Semester2/HonoursProject")
deaths_marital <- read_csv("Data/Deaths_by_Marital_Status.csv") 
```
...
```{r}
# somehow create a function that gets these linear models for each risk class
# 1. create function that filters data
# 2. create function that creates linear model for filtered data
# 3. create function that circulates through, stores linear models and respective sum of squared residuals

# compare to sum of squared residuals of large model
...
```{r}
## using the better model to predict deaths in the future

# training data will be from 1991-2015
# testing from 2016 - 2018
...
Death_modelPlot_TrainTest <- function(geo, age, sex, ms, train_d, test_d){
  ...
  plot(model_train_data$Year, model_train_data$Deaths, xlab = "Year", xlim=c(1991, 2018),
       ylab = "Deaths", ylim = ybounds, main="Model Vs Actual")
...
```{r}
# Expected_Deaths <- array(NA, dim=c(length(geos), length(ages), length(ms_s) , length(sexes)))
...
```{r}
RMSE(predictions, model_test_data$Deaths)
```
